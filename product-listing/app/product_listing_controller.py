...
class ProductListingController:
    ...
    def get_product_listings(current_user=Depends(get_current_user), self=Depends(ProductListingDep)):
    ...
    def get_product_listing_by_id(
        product_listing_id: str, current_user=Depends(get_current_user), self=Depends(ProductListingDep)
    ):
